    ...
    <div class="p-4 md:p-8 max-w-7xl mx-auto space-y-6">
      <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <p class="text-slate-500 text-sm mt-0.5">
          Generate patient or site analytics reports.
        </p>
      </div>
      ...
      <div class="flex flex-wrap gap-2 bg-white p-2 rounded-2xl border border-slate-200 shadow-sm">
        <button
          ...
          :class="activeTab === 'patient' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'"
      ...
      <Transition name="fade" mode="out-in">
        <div v-if="activeTab === 'patient'" key="patient">
          <ReportGenerator />
        </div>
        <div v-else key="site">
          <SiteReportGenerator />
